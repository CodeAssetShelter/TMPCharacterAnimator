...
public sealed class TMP_CharacterAnimator : MonoBehaviour
{
    public enum EffectType
    {
        None, Wave, Bounce, Floating, Drift, Fall, Shuffle, Spin, Flip,
        Tilt, Pulse, Pop, WobbleScale, Rainbow, Flash, GradientScroll,
        Typewriter, Fade, DragWave
    }

    [SerializeField] private EffectType effect = EffectType.Typewriter;
    ...
    {
        if (!_tmp) _tmp = GetComponent<TextMeshProUGUI>();
        if (Application.isPlaying) SetEffect();
    }
